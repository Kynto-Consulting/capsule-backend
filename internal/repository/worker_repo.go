...
func (r *WorkerRepository) Create(ctx context.Context, w *domain.Worker) (*domain.Worker, error) {
	...
	err := r.pool.QueryRow(ctx, `
		INSERT INTO workers (project_id, name, command, replicas, restart_policy)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, project_id, name, command, replicas, status, container_id, restart_policy, created_at, updated_at`,
		w.ProjectID, w.Name, w.Command, w.Replicas, w.RestartPolicy,
	).Scan(&out.ID, &out.ProjectID, &out.Name, &out.Command, &out.Replicas,
		&out.Status, &out.ContainerID, &out.RestartPolicy, &out.CreatedAt, &out.UpdatedAt)
	if err != nil {
...
func (r *WorkerRepository) GetByID(ctx context.Context, id uuid.UUID) (*domain.Worker, error) {
	var out domain.Worker
	err := r.pool.QueryRow(ctx, `
		SELECT id, project_id, name, command, replicas, status, container_id, restart_policy, created_at, updated_at
...
func (r *WorkerRepository) ListByProject(ctx context.Context, projectID uuid.UUID) ([]*domain.Worker, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&w.ID, &w.ProjectID, &w.Name, &w.Command, &w.Replicas,
			&w.Status, &w.ContainerID, &w.RestartPolicy, &w.CreatedAt, &w.UpdatedAt); err != nil {
